...
export const register = catchAsyncErrors(async (req, res, next) => {
  try {
    const { name, email, password, gstNumber, businessName } = req.body;
    if (!name || !email || !gstNumber || !businessName || !password) {
    ...
    if (!gstRegex.test(gstNumber)) {
      return next(new ErrorHandler("Invalid GST Number format.", 400));
    }

    const isRegisteredGstNumber = await User.findOne({
      gstNumber,
      accountVerified: true,
    });
    if (isRegisteredGstNumber) {
    ...
    if (password.length < 8 || password.length > 16) {
      return next(
        new ErrorHandler(
          "Password must be between 8 to 16 character long.",
          400
        )
      );
    }
...
export const forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  const message = generateForgotPasswordEmailTemplete(
    resetPasswordUrl,
    user?.name
  );
